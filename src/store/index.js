...
export const store = new Vuex.Store({
  ...
  mutations: {
    favoriteProduct(state, payload) {
      ...
      if (
        state.user.favoritedProducts.findIndex(product => product.id === id) >=
        0
      ) {
        return;
      }
    ...
    filterAllProducts(state, payload) {
      const slicedArray = [...state.allProducts].filter(el =>
        payload.includes(el.id)
      );
      const hash = {};
    ...
    infiniteScroll({ commit, getters }) {
      ...
      firebase
        ...
        .then(data => {
          ...
          if (
            getters.lastKeys[getters.lastKeys.length - 1] ===
            getters.lastKeys[getters.lastKeys.length - 2]
          ) {
            commit("setLoading", false);
